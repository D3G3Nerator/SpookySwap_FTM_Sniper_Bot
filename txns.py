...
class TXN():
    def __init__(self, token_address, quantity):
        ...
        self.MaxGasInFTM, self.gas_price = self.setupGas()
    ...
    def setupGas(self):
        with open("./Settings.json") as f:
            keys = json.load(f)
        return keys['MaxTXFeeFTM'], int(keys['GWEI_GAS'] * (10**9))
    ...
    def setup_swapper(self):
        swapper_address = Web3.toChecksumAddress("0xdA19096CCecFEEE1da30021c17a33819dE6E11F1") #FTM
    ...
    def estimateGas(self, txn):
        ...
        maxGasFTM = Web3.fromWei(gas * self.gas_price, "ether")
        print(style.GREEN + "\nMax Transaction cost " + str(maxGasFTM) + " FTM" + style.RESET)

        if maxGasFTM > self.MaxGasInFTM:
            print(style.RED +"\nTx cost exceeds your settings, exiting!")
            raise SystemExit
        return gas


    def getOutputfromFTMtoToken(self):
    ...
    def getOutputfromTokentoFTM(self):
